...
def prepare_df_model(target, inds, df0, df_clu):
    ...
    forecast_end = str(last_quarter + pd.DateOffset(months=3)).split()[0]
...
def short_term_model_accuracy_1q(indicator_id, analytical_for_test, target, forecast_start, forecast_end,citizen_):
    ...
    # Test
    # needed_cols = ['PMI_Whole Economy_Abu Dhabi_Three month Moving Average_Overall PMI_BACKLOGS','UEMSM1Y Index']
    # needed_cols.append(target)
    # analytical_for_test = analytical_for_test[needed_cols]
    # Test end
    ...
    if not os.path.isdir(wdir):
        os.mkdir(wdir)
    input_data_export.to_excel(wdir+f"Model_Input_of_{indicator_id}_{target}_1Q.xlsx") #test
    ...
    # experiment_id = ml_flow_params[citizen_][target]
    # mlflow.start_run(experiment_id=experiment_id)
    # mlflow.sklearn.autolog()

    run_model = 'AutoML'  # Initially Lasso was used 

    if run_model == 'lasso':
        print("-----RUNNING LASSO REGRESSION---------------")
        model = Lasso(alpha=0.005)
        # Fit the model into our training feature
        ...
        model.fit(X_data, y_data,task='regression',estimator_list = ['rf', 'xgboost', 'extra_tree', 'xgb_limitdepth'],time_budget=60)
        # model.fit(X_train, y_train,task='regression',estimator_list = ['rf', 'extra_tree'],time_budget=5)

        print("--------FEATURE IMPORTANCE------------------")
        print(model.model.estimator.feature_importances_)
        print(X_data.columns)
        importances = model.model.estimator.feature_importances_
...
def forecast_accuracy_1q_func(quarters, analytical_df_var2, indicator_id, target):
    ...
    for Q_forecast in quarters[-4:]:
        ...
        accuracies.append(Acc_l1)
        print('Accuracy Quarter: ', str(Q_forecast), ' = ', Acc_l1 * 100, '%')
