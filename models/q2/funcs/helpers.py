...
def prepare_output(df_all_pred, df_clu, citizen_id, target_reg, forecast_accuracy_1q, forecast_end):
    ...
    # get max run_seq_id and table format
    ...
    # Edit forecast rows
    next_quarter_start = pd.Timestamp(params.forecast_start) + pd.DateOffset(months=3)
    forecast_1Q = next_quarter_start + pd.offsets.MonthEnd(0)
    forecast_1Q = forecast_1Q.strftime('%Y-%m-%d')
    
    ind = df_out_pred[df_out_pred.obs_dt.isin([forecast_1Q,forecast_end])].index
    df_out_pred.loc[ind, 'opt'] = 1
    df_out_pred.loc[ind, 'type']='FORECAST UNEMPLOYMENT'
    tmp = df_all_pred.reset_index()
    df_out_pred.loc[ind,'parameter_combo_id'] = tmp[tmp.OBS_DT.isin([forecast_1Q,forecast_end])]["COMBO"]
